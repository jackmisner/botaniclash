...
describe("authentication service", () => {
  describe("login", () => {
    ...
    test("returns the token if the request was a success", async () => {
      ...
      const token = await login(testUsername, testPassword);
  ...
  describe("signup", () => {
    test("calls the backend url for a token", async () => {
      ...
      await signup(testUsername, testPassword);
    ...
    test("returns nothing if the signup request was a success", async () => {
      ...
      const token = await signup(testPassword, testUsername);
    ...
    test("throws an error if the request failed", async () => {
      ...
      try {
        await signup(testPassword, testUsername);
      } catch (err) {
        expect(err.message).toEqual(
